 ...
 */
function getSignedUrlAsync(cfUrl, params, callback) {
  // Stringify private key if supplied as a file path.
  if (params.privateKeyPath) {
    fs.readFile(params.privateKeyPath, function(err, data) {
      if (err) {
        throw err;
      }
      params.privateKeyString = data.toString('ascii');
      params.privateKeyPath = null;
      callback(getSignedUrl(cfUrl, params));
    });
  } else {
    callback(getSignedUrl(cfUrl, params));
  }
...
exports.getSignedUrlAsync = getSignedUrlAsync;
